...
message LogBatch { string source=1; repeated LogLine lines=2; }  // job-id|runtime|agent
...
message JobUpdate {
  ...
  enum Phase { PENDING=0; RUNNING=1; SUCCEEDED=2; FAILED=3; CANCELLED=4; }
...
### 하행 명령 (At-least-once + 멱등성)
```
CP: Command{command_id} 전송 → ack 대기, 타임아웃 시 재전송
...
## 미해결/후속
- RunJob / PushBundle 페이로드 상세 — D. Job 오케스트레이션에서 확정
- seq 영속화 범위(연결 단위 vs 노드 단위) — C. 데이터모델과 함께 확정
